...
# Candidate Savant team-OAA CSV endpoints (parsing is column-name driven, so
# either shape works; first one that returns parseable CSV wins).
SAVANT_URLS = [
...
def build_feed(lines, season, team_def_runs):
    ...
    for ln in lines:
        ...
        def war_of(rate):
            return (rep_ra9 - rate) / rpw * ip / 9.0
    ...
    return {
        "meta": {
            ...
            "method": (
                ...
                "RA9-based pole with a bWAR-style team-defense adjustment "
                "(war_ra9_adj; war_ra9 is unadjusted). No park factors, static "
                "runs-per-win. WAR at any blend = lerp(war_fip, war_ra9_adj, lambda)."
            ),
        },
        "league": {
            "ra9": round(lg_ra9, 3),
